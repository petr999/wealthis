...
module.exports = function( host, user, password, database ){
  ...
  this.existPersonByRow = async ( dbh, row ) => {
    ...
    const [ reqresId, email, first_name, last_name, avatar ] = 
  ...
  this.updatePerson = async ( dbh, row ) => {
    ...
    const sql = 'update reqresUsers set '
      + ( 'reqresId first_name last_name avatar'.split( / +/ ).map( val => {
            const rv = val + ' = ? '
            return rv
          } ).join( ', ' )
        )
      + ' where email = ?';
    await dbh.query( sql, [ reqresId, first_name, last_name, avatar,
      email,
    ] )
  }
  ...
  this.insertPerson = async ( dbh, row ) => {
    ...
    const sql = 'insert into reqresUsers  ( '
  ...
  this.retrievePersons = async searchString => {
    ...
    if( searchString.length ){
      ...
      sql += ' where match( first_name, last_name ) '
